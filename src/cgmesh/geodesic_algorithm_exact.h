...
private:
	...
	unsigned long m_queue_max_size;			//used for statistics
	unsigned long m_iterations;			//used for statistics
									   ...
									   std::vector<SurfacePoint>* stop_points)
{
	...
	while(!m_queue.empty())
	{
		m_queue_max_size = std::max((long unsigned int)m_queue.size(), m_queue_max_size);
