...
static int Between (const Vector2f& a, const Vector2f& b, const Vector2f& c)
{
...
//
unsigned int line_ellipse_intersection (const float* line_start, const float* line_end, const float* ellipse_center, const float* ellipse_radius, float* res1, float* res2)
{
	// translate center of the ellipse to the origin
	Vector2f center (ellipse_center[0], ellipse_center[1]);
	Vector2f l_start = Vector2f (line_start[0], line_start[1]) - center;
	Vector2f l_end   = Vector2f (line_end[0], line_end[1]) - center;

	// define the line with the following parametric equation : P = l_start + t * l_dir
	Vector2f l_dir = l_end - l_start;
	l_dir.Normalize ();
	...
	if (discriminant > 0.)
	{
		...
		res1[0] = ellipse_center[0] + l_start[0] + t1*l_dir[0];
		res1[1] = ellipse_center[1] + l_start[1] + t1*l_dir[1];
		res2[0] = ellipse_center[0] + l_start[0] + t2*l_dir[0];
		res2[1] = ellipse_center[1] + l_start[1] + t2*l_dir[1];
		return 2;
	}
	else if (discriminant == 0.)
	{
		float t = -.5*b/a;
		res1[0] = ellipse_center[0] + l_start[0] + t*l_dir[0];
		res1[1] = ellipse_center[1] + l_start[1] + t*l_dir[1];
		return 1;
	}
