...
// Export OBJ de la forme `id` avec UN GROUPE PAR BLOC CONNEXE (`o block_NNNN_...`).
// Renvoie "" si la forme n'est pas une brique de blocs pixelises.
//
// Un seul fichier plutot que N : aucune dependance zip cote JS, et tout slicer
// sait separer les groupes d'un OBJ. Les indices OBJ etant globaux au fichier et
// numerotes a partir de 1, on cumule un decalage d'un maillage a l'autre.
std::string exportPixelBlocksObj(int id)
{
    ...
    if (meshes.empty()) return std::string();

    std::string out = "# generated by maker (cgmesh wasm) - one group per block\n";
    unsigned int base = 1;   // OBJ indexe a partir de 1
    for (Mesh* m : meshes)
    {
        if (!m) continue;
        Material* mat = m->GetMaterial(0);
        const std::string name = mat ? mat->GetName() : std::string("block");
        out += "o "; out += name; out += '\n';
        ...
        {
            out += "f ";
            out += std::to_string(tris[t]     + base); out += ' ';
            out += std::to_string(tris[t + 1] + base); out += ' ';
            out += std::to_string(tris[t + 2] + base); out += '\n';
        }
        base += (unsigned int)nv;
...
EMSCRIPTEN_BINDINGS(maker)
{
    ...
    emscripten::function("exportPixelBlocksObj",   &exportPixelBlocksObj);
